...
#include <SPI.h>
#include "SPI2C.h"
#include "MPU6X_PARTIAL.h"
 
#define WR comP->setSPI2CConfig(i2cConfig); comP

//SPI2C_config mpu6xxx32Config = { SPI2C_type_e::SPI2C_I2C,0x68,SPI2C_I2C_CLOCK_400, SPI2C_NOT_USED,SPI2C_NOT_USED };
SPI2C_config mpu6xxx32Config = { SPI2C_type_e::SPI2C_SPI_TRANSACTIONAL,10,1000000,SPI_MODE3,MSBFIRST };
MPU6XXX32::MPU6XXX32()
{
}
...
uint8_t MPU6XXX32::getDeviceID()
{
 ...
 WR->readBits(  MPU6050_RA_WHO_AM_I | spiMode  ?  0x80 : 0x00, MPU6050_WHO_AM_I_BIT, MPU6050_WHO_AM_I_LENGTH, &buffer[0]);
...
void MPU6XXX32::getAcceleration(int16_t * x, int16_t * y, int16_t * z)
{
	...
	WR->read(spiMode ? MPU6050_RA_ACCEL_XOUT_H | 0x80 : MPU6050_RA_ACCEL_XOUT_H, buffer, 6);
...
{
	WR->read(   spiMode ? MPU6050_RA_GYRO_XOUT_H | 0x80 : MPU6050_RA_GYRO_XOUT_H,   buffer,6);
	*x = (((int16_t)buffer[0]) << 8) | buffer[1];
	*y = (((int16_t)buffer[2]) << 8) | buffer[3];
	*z = (((int16_t)buffer[4]) << 8) | buffer[5];
}
